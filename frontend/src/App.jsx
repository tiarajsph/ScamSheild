...
function App() {
  ...
  async function analyzeText(text) {
    ...
    try {
      ...
      setIsScam(
        data.prediction === "scam"
          ? "scam"
          : data.prediction === "likely spam"
            ? "likely"
            : "legit",
      );
  ...
  async function analyzeQr(file) {
    ...
    try {
      ...
      if (data.prediction) {
        ...
        setPrediction("No QR code found");
        setIsScam(null);
        setConfidence(0);
        setExplanation([]);
      } else {
  ...
  return (
    <div className={`app-shell ${bgClass}`}>
      ...
      <div className="app-body">
        <div
          className={`card ${cardClass} ${activeTab === "qr" ? "card-compact" : ""}`}
        >
          {/* Input tab */}
          {activeTab === "text" ? (
            <TextAnalysis
